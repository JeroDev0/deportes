...
function EditSponsorProfile() {
  ...
  useEffect(() => {
    if (form.country) {
      ...
      fetch(`https://secure.geonames.org/searchJSON?country=${form.country}&featureClass=P&maxRows=1000&username=jerodev0`)
        .then((res) => res.json())
        .then((data) => {
...
const handleLogoChange = (e) => {
  ...
  formData.append("athletes", JSON.stringify(form.athletes || []));
  formData.append("clubs", JSON.stringify(form.clubs || []));

  // archivo
  if (form.logo instanceof File) {
    formData.append("logo", form.logo); // 👈 KEY EXACTA: "logo"
  }
  ...
  try {
    ...
    console.log("✅ Sponsor actualizado:", data);
